...
// ── Type checking guard ────────────────────────────────────────
export function isStructuredInput(input) {
    if (typeof input !== "object" || input === null)
        return false;
    const obj = input;
    return (("lot_width" in obj || "lot_depth" in obj || "rooms" in obj) &&
        typeof input === "object");
}
...
function extractRooms(text) {
    ...
    for (const { pattern, type } of ROOM_PATTERNS) {
        ...
        while ((m = global.exec(text)) !== null) {
            ...
            const overlaps = claimedRanges.some(r => matchStart < r.end && matchEnd > r.start);
            if (overlaps)
                continue;
 ...
 */
export function parseHouseDescription(text) {
    ...
    if (rooms.length === 0) {
        ...
        const hasLiving = rooms.some(r => ["living_room", "living_dining", "living_kitchen"].includes(r.type));
 ...
 */
export function resolveHouseSpec(input, overrides = {}) {
    let base;
...
function mergeWithDefaults(partial) {
    const rooms = (partial.rooms ?? DEFAULT_ROOMS).map(r => ({
        ...r,
