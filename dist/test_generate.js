...
async function run(label, input, overrides = {}) {
    console.log(`\n${"═".repeat(70)}`);
    console.log(`TEST: ${label}`);
    console.log("═".repeat(70));
    const spec = resolveHouseSpec(input, overrides);
    console.log("\n── HouseSpec ─────────────────────────────────────────────────────────");
    ...
    if (lspLines.length > 60)
        console.log(`  ... (${lspLines.length - 60} more lines)`);
}
(async () => {
    // Test 1: NLP free-text
    await run("NLP_casa3d2b", "casa de 3 dormitorios, 2 baños, cocina-sala abierta, lote 12x9");
    // Test 2: Structured override (client request)
    await run("structured_client", "client request", {
        lot_width: 10,
        lot_depth: 8,
        rooms: [
