...
function placeDoor(fromRoom, toRoom, wallInt, blockPath, doorWidth = RULES.interior_door_width) {
    ...
    if (Math.abs(toRoom.x1 - sharedX) < wallInt + 0.01) {
        ...
        if (overlapY2 - overlapY1 < doorWidth)
            return null;
    ...
    if (Math.abs(toRoom.y1 - sharedY) < wallInt + 0.01) {
        ...
        if (overlapX2 - overlapX1 < doorWidth)
            return null;
