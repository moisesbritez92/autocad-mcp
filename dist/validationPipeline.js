 ...
 */
export async function runSemanticPipeline(contextJsonPath, projectRoot) {
 ...
 */
export function extractValidation(enriched) {
    const rooms = (enriched.rooms ?? []);
    const openings = (enriched.openings ?? { doors: [], windows: [] });
    ...
    const agent_hints = (enriched.agent_hints ?? {});
    const metrics = {
        total_area: rooms.reduce((sum, r) => sum + (r.area ?? 0), 0),
        wall_count: (enriched.walls ?? []).length,
 ...
 */
export async function validateDrawing(contextJsonPath, projectRoot) {
