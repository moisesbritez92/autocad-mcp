...
import type { Layout, PlacedRoom, DoorSpec, WindowSpec } from "./types.js";
...
function toDwgPath(p: string): string {
  // AutoLISP needs forward slashes
  return p.replace(/\\/g, "/");
}

// Centroid of a placed room
function centroid(r: PlacedRoom): [number, number] {
...
function buildFloor(layout: Layout): string {
...
function buildWallShell(layout: Layout): string {
...
function buildRoomVoids(rooms: PlacedRoom[]): string {
  let s = `\n  ;; ── Room voids ────────────────────────────────────────────────────────\n`;
...
function buildOpenings(doors: DoorSpec[], windows: WindowSpec[]): string {
  ...
  const windowsOnMain = windows.filter(w =>
    !["N", "S"].includes(w.facing) ||
...
function buildBlockInserts(doors: DoorSpec[], windows: WindowSpec[]): string {
  let s = `\n  ;; ── Door block inserts ────────────────────────────────────────────────\n`;
...
function buildDimensions(layout: Layout): string {
  const { lot_width: w, lot_depth: d, include_terrace, terrace_depth } = layout;
  let s = `\n  ;; ── Dimensions ────────────────────────────────────────────────────────\n`;
...
function buildSave(outputPath: string): string {
 ...
 */
export async function writeLSPToTemp(
  lspCode: string,
  tempDir: string
): Promise<string> {
 ...
 */
export function buildRunScript(lspPath: string): string {
 ...
 */
export async function writeRunScript(lspPath: string): Promise<string> {
